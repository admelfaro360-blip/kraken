...
import { fetchUsers, getUserById } from '../lib/storage';
    ...
    try {
      ...
      try {
        ...
        if (username.includes('@')) {
          ...
          console.log("Login exitoso. Auth UID:", userCredential.user.uid);
          
          // Verificar si el usuario existe en Firestore
          try {
            const userDoc = await getUserById(userCredential.user.uid);
            if (!userDoc) {
              console.error("ALERTA CRÍTICA: El usuario se autenticó en Auth, pero no tiene documento en la colección 'users' de Firestore. UID:", userCredential.user.uid);
              toast.error("Tu usuario no tiene permisos registrados en la base de datos.");
            } else {
              console.log("Documento de usuario encontrado en Firestore:", userDoc);
            }
          } catch (docErr) {
            console.error("Error al verificar documento de usuario en Firestore:", docErr);
          }
        ...
        console.error("Fallo en Firebase Auth (Email/Pass):", authErr.code, authErr.message);
